...
final as (
    select 
        a.team,
        c.date,
        case when coalesce(date - lag(date) OVER (PARTITION BY a.team ORDER BY date), 5)::numeric - 1 > 4 then 4
    ...
    from my_cte c
    left join {{ ref('staging_seed_team_attributes') }} a on a.team_acronym = c.team 
)
